...
| `diff_view` | `"rendered"` | 全画面 diff の 3 表現のうち、開いた時にどれを見せるか。`"rendered"` は Markdown ファイルの diff を装飾ブロックで描く — 削除は赤/dim、追加は緑、変更後は琥珀 — なので編集の「何が消えたか」が最終結果だけでなく見える(Markdown 以外のファイルは `"source"` に丸める。ブロック単位の diff が無いため)。`"source"` は従来の unified/split diff。`"preview"` はそのファイルの通常プレビュー(変更ガター付き)。`R` キーで、対象ファイルが持つ表現だけを巡回する(素のテキストファイル: `"source"` ⇄ `"preview"` のみ・非テキスト: 表現が無く、ヒントも出ない)。不明な値は `"rendered"` にフォールバック。 |
...
- **Git**: `open_git_view`(`o`)・`open_git_diff_cursor`(`d`)・`git_stage`・`git_unstage`・`git_stage_all`・`git_unstage_all`・`git_discard`・`git_commit`・`git_open_log`・`git_open_graph`・`git_open_branches`・`git_launch_tool`(`!`・変更ハブ内)・`cycle_diff_layout`・`cycle_diff_view`(`R`・全画面 diff 内: 3 表現 — ソース/整形/プレビュー — を巡回・`[ui] diff_view`)・`git_copy_*`・`branch_*`
