...
図の描かれ方を変える `[ui]` の設定は 3 つあります。この節の絵はすべて**同じ flowchart**
（このページの冒頭にあるキー入力の図）で、設定を 1 つだけ動かして描き直したものです。
違って見えるものは、キャプションが示す設定だけが原因です。

### `mermaid_routing` — 辺の配線方法
...
この設定を読むのは flowchart 自身の辺だけで、`konoma-orthogonal` では無効です
（そのモードに曲線は無く、直角の区間だけがあります）。曲線の指定は 3 段階
（弱い順）: この設定 < 図側の `%%{init: {"flowchart": {"curve": "..."}}}%%`
ディレクティブ < 特定の辺だけを指す `linkStyle <n> interpolate <curve>`。

<MermaidSettingGrid lang="ja" option="mermaid_curve" group={manifest.settings.curve} columns="two" />

### `mermaid_theme` — 配色

背景は常に透過なので、図は端末の背景に馴染みます。`konoma-orthogonal` のときは
無効です — そのモードは常に konoma 独自の設計配色を使います。

<MermaidSettingGrid lang="ja" option="mermaid_theme" group={manifest.settings.theme} />
