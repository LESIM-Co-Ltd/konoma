...
| `mermaid_rows` | `24` | Target height (rows) of an inline mermaid diagram inside Markdown — scales up too: diagrams re-rasterize at the needed density and stay sharp (width-capped, aspect-preserving). The initial view also fits the viewport: shorter windows shrink the diagram so the whole block is visible. 0/invalid falls back to the default. |
| `math` | `"image"` | How LaTeX math renders. `"image"` (default) rasterizes `$…$` / `$$…$$` in-process via RaTeX (pure Rust, KaTeX quality, no browser/Node) and shows each as an inline image (inline math is lifted onto its own line since a terminal cannot place an image mid-text; display math is centered). `"text"` leaves the raw LaTeX as plain text. Non-graphics terminals and render failures degrade to the raw LaTeX automatically. |
