...
| `diff_view` | `"rendered"` | Which of the full-screen diff's 3 presentations it opens showing. `"rendered"` draws a Markdown file's diff as decorated blocks — removed content red/dim, added green, changed-to amber — so an edit's deletions stay visible, not only its final result (falls back to `"source"` for a non-Markdown file, which has no block-diff renderer). `"source"` is the classic unified/split diff. `"preview"` is the file's ordinary preview with a change gutter. `R` cycles between whichever presentations the file being diffed actually has (a plain text file: `"source"` ⇄ `"preview"` only; a non-text file: none, and no hint is shown). An unrecognized value falls back to `"rendered"`. |
...
- **Git**: `open_git_view` (`o`), `open_git_diff_cursor` (`d`), `git_stage`, `git_unstage`, `git_stage_all`, `git_unstage_all`, `git_discard`, `git_commit`, `git_open_log`, `git_open_graph`, `git_open_branches`, `git_launch_tool` (`!`, in the changes hub), `cycle_diff_layout`, `cycle_diff_view` (`R`, in a full-screen diff: cycles the 3 presentations — source/rendered/preview — `[ui] diff_view`), `git_copy_*`, `branch_*`
