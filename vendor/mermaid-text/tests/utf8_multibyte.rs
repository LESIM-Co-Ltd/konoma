...
// ---- konoma-local addition ----
// This file (as of mermaid-text-0.57.0) covers sequence/flowchart/gantt/timeline/journey but not
// `classDiagram`. Unlike those five, `class.rs`'s `strip_keyword_prefix_ci` was never migrated to
// share `common::strip_keyword_prefix` (which is what actually fixed this bug class in 0.9.0 for
// everything else) and still slices `line[..keyword.len()]` without a char-boundary check --
// confirmed to still panic at this version before konoma's local patch (see the doc comment on
// `strip_keyword_prefix_ci` in src/parser/class.rs). A relation line with a two-or-more-byte-per-char
// non-ASCII source class name, at top level (not inside a `class Name { ... }` body -- those lines
// go through `parse_member` instead, which never hits this function), lands the byte offset for at
// least one of the five keywords `reject_unsupported` checks (direction/note/link/click/namespace,
// lengths 9/4/4/5/9) mid-character. This is a regression guard for the local patch, not (yet) a
// claim about upstream: if a future vendor sync makes this pass with the patch reverted, the patch
// can be dropped.
#[test]
fn class_diagram_relation_with_cjk_class_names_does_not_panic() {
    let src = "classDiagram\n動物 <|-- 犬\n";
    let r = std::panic::catch_unwind(|| mermaid_text::render(src));
    assert!(r.is_ok(), "classDiagram render panicked on CJK class names:\n{src}");
}
