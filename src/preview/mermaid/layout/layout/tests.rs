...
fn acyclic_breaks_cycles() {
    ...
    let cycles = crate::preview::mermaid::layout::graph::alg::find_cycles(&g);
    assert!(
        cycles.is_empty(),
...
fn nesting_run_connects_disconnected_graph() {
    ...
    let comps = crate::preview::mermaid::layout::graph::alg::components(&g);
    assert_eq!(comps.len(), 2);
    nesting_graph::run(&mut g);
    let comps2 = crate::preview::mermaid::layout::graph::alg::components(&g);
...
fn parent_dummy_chains_no_parent_if_both_have_no_parent() {
    ...
    g.set_node("b".to_string(), Some(NodeLabel::default()));
    let d1 = NodeLabel {
        edge_obj: Some(crate::preview::mermaid::layout::graph::Edge::new("a", "b")),
...
fn parent_dummy_chains_handles_nested_subgraphs() {
    ...
    for i in 0..5 {
        ...
        if i == 0 {
            d.edge_obj = Some(crate::preview::mermaid::layout::graph::Edge::new("a", "b"));
        }
...
fn greedy_fas_returns_empty_for_empty_graph() {
    let g: Graph<NodeLabel, EdgeLabel> = Graph::new();
    let weight_fn = |e: &crate::preview::mermaid::layout::graph::Edge| -> i32 {
...
fn greedy_fas_returns_single_edge_for_simple_cycle() {
    ...
    assert!(crate::preview::mermaid::layout::graph::alg::find_cycles(&g).is_empty());
}
