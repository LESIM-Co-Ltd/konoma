...
use crate::preview::mermaid::layout::layout::types::{
    Align, BorderType, EdgeLabel, GraphLabel, LabelPos, NodeLabel,
};
use crate::preview::mermaid::layout::layout::util::build_layer_matrix;
            ...
            for i in lo..=hi {
                if let Some(w) = ws.get(i) {
                    ...
                    if align.get(v.as_str()).map(|a| a.as_str()) == Some(v.as_str()) {
                        if prev_idx < pos_w as isize {
                            if !has_conflict(conflicts, v, w) {
...
pub(crate) fn horizontal_compaction(
    ...
    let pass2 = |block_g: &Graph<(), f64>, xs: &mut PositionMap, elem: &str| {
        ...
        if let Some(node) = g.node(elem) {
            if min != f64::INFINITY {
                if node.border_type != Some(border_type) {
...
mod tests {
    ...
    use crate::preview::mermaid::layout::graph::Graph;
    use crate::preview::mermaid::layout::layout::types::{
        EdgeLabel, GraphLabel, LabelPos, NodeLabel,
    };
    use crate::preview::mermaid::layout::layout::util::build_layer_matrix;
    use std::collections::BTreeMap;
