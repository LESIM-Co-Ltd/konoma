...
fn cross_validate_all_cases() {
    ...
    // Every divergence is reported above before the assertion, so a failing run
    // names the case, the node and the two coordinates rather than just a count.
    // (Upstream's comment here claimed this only reported and did not assert; it
    // has asserted since the baseline was added -- see PROVENANCE.md §7.)
    if total_fail > 0 {
        panic!(
