...
// GUARDS: the block-model Markdown renderer (`render::render_doc`, the renderer every real preview
// now dispatches to — see `render_markdown_with_images`'s own doc comment) is not dramatically slower
// or more allocation-hungry than the pre-`md-block-walk` legacy renderer
// (`render_markdown_with_images_legacy`) it replaced, on the same synthetic large document `md_doc`
// (headings/paragraphs/links/a big code fence — the identical fixture the sibling guard above already
// uses, so both stay hermetic — no `~/.cargo/registry` dependency, unlike `app::md_real_file_sweep_\
// tests::real_markdown_corpus_perf_probe`, the `#[ignore]`d real-file investigation this guard's own
// threshold was calibrated from). Measured there, on the sweep's own five largest real files
// (150-207KB each, real crate CHANGELOGs): the new renderer was consistently *faster* (0.70-0.78x
// wall-clock) and allocated *less* (0.84-0.87x) than legacy — never worse on either axis. `3.0x` here
// is deliberately generous headroom above that "never worse" baseline, not a tight bound around it —
// this guard exists to catch a gross regression (an accidental re-parse, an unbounded clone), not to
// enforce the real-file measurement's own specific ratio, which will drift with document shape.
#[test]
fn render_doc_scales_like_legacy_on_a_large_synthetic_document() {
    use crate::preview::markdown::model::Doc;
    use crate::preview::markdown::render::render_doc;
    use crate::preview::markdown::{
        render_markdown_with_images_legacy, CodeStyle, ImageSlot, MathSlot, MermaidSlot,
        DEFAULT_TASK_STATES,
    };
    ...
    // Warm the "rust" fence's grammar outside the measurement (same reasoning as the sibling guard).
    let _ = render_new(&md_doc(1));
    let _ = render_legacy(&md_doc(1));

    let doc200 = md_doc(200); // the sibling guard's own "large" size — big enough to be meaningful,
                              // small enough to keep this test fast in the normal suite.
    assert!(!render_new(&doc200).is_empty());
    assert!(!render_legacy(&doc200).is_empty());

    let new_alloc = crate::mem_tests::allocated_by(|| {
        assert!(!render_new(&doc200).is_empty());
    });
    let legacy_alloc = crate::mem_tests::allocated_by(|| {
        assert!(!render_legacy(&doc200).is_empty());
    });
    assert!(
        (new_alloc as f64) < (legacy_alloc as f64) * 3.0,
        "block-model レンダラがレガシーの3倍を超えて確保した(回帰?): new={new_alloc} legacy={legacy_alloc}"
    );
}
